...
-------------------------------------------------------------------------------
20201125	Lucky		First release(v0.1)
*/
...
bool GeoTiffFile::open(const QString & filePath)
{
	...
	_type = _nlevels >= 3 ? MULTI_FILE : SINGLE_FILE;
...
void GeoTiffFile::deleteRasterData(float ** d) const
{
	if (d == 0) return;
...
SatImageFileType GeoTiffFile::fileType() const
{
	return _type;
}
